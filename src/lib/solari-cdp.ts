...
class CdpConnection {
  ...
  private constructor(socket: WebSocket) {
    ...
    socket.addEventListener("close", () => {
      for (const [, waiter] of this.pending) waiter.reject(new Error("Solari CDP connection closed unexpectedly"));
      this.pending.clear();
    });
  }
  ...
  static async connect(endpoint: string) {
    ...
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("Timed out connecting to Solari CDP")), 10000);
      socket.addEventListener("open", () => { clearTimeout(timer); resolve(); }, { once: true });
      socket.addEventListener("error", () => { clearTimeout(timer); reject(new Error("Failed to connect to Solari CDP")); }, { once: true });
    });
...
export async function createSolariSession(apiKey: string): Promise<SolariSession> {
  ...
  catch { throw new Error(`Solari session creation returned invalid JSON (HTTP ${response.status})`); }
  if (!response.ok) throw new Error(`Solari POST /sessions failed: ${response.status} ${raw || response.statusText}`);
  const id = payload.id ?? payload.sessionId;
  const cdpEndpoint = payload.cdpEndpoint ?? (payload.wsEndpoint ? deriveCdpEndpoint(payload.wsEndpoint) : undefined);
  if (!id || !cdpEndpoint) throw new Error("Solari session response did not include a session id and CDP endpoint");
  return { id, cdpEndpoint };
}

export async function releaseSolariSession(apiKey: string, id: string) {
  try {
    await fetch(`https://api.getsolari.com/sessions/${encodeURIComponent(id)}`, {
      method: "DELETE", headers: { Authorization: `Bearer ${apiKey}` }, cache: "no-store",
    });
  } catch (error) { console.error("Failed to release Solari session", error); }
}

export async function extractWaybillFields(cdpEndpoint: string, url: string, waybill: string, fieldNames: string[]) {
  ...
  try {
    ...
    while (Date.now() < deadline) {
      const namesJson = JSON.stringify(fieldNames);
      const expression = `(() => {
        const root = document.querySelector(${JSON.stringify(`[data-waybill="${waybill}"]`)});
        if (!root) return null;
        const names = ${namesJson};
        return Object.fromEntries(names.map((name) => [name, (root.querySelector(`[data-field="${name}"]`)?.textContent ?? "").trim()]));
      })()`;
      const evaluated = await cdp.send("Runtime.evaluate", { expression, returnByValue: true, awaitPromise: true }, targetSessionId);
      const value = evaluated?.result?.value;
      if (value) return value as Record<string, string>;
      await new Promise((resolve) => setTimeout(resolve, 250));
    }
    throw new Error(`Waybill ${waybill} was not found after navigating to ${url}`);
  } finally { cdp.close(); }
}
